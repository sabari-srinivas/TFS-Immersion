...
Output a root-cause map, test plan, and privacy constraints.`,
  },
  {
    step: 3,
    label: "Ideate",
    text: `Generate a wide set of possible AI-driven ideas for the root causes diagnosed above — at least 15, each one line. Generate first: do not group, label, or rank while generating.

Now cluster the ideas you just generated by MECHANISM OF ACTION — what each idea actually changes in the work (e.g. removes a step entirely, redesigns how information reaches a person, augments a person's judgment in the moment, re-times when work happens, makes something invisible visible). Derive exactly 4 clusters from the ideas themselves. Name each cluster in the TSF Group's own operational vocabulary — generic technology labels ("AI", "Automation", "ML", "Process") are not acceptable cluster names. Under each cluster heading, restate its ideas compactly — number plus a shortened version of its one-line description — so every cluster reads on its own without scrolling back to the flat list. State each cluster's rationale exactly once, and do not repeat the idea list anywhere else in the response. Mark the single strongest idea in each cluster with one line on why it leads.

Score each on Impact × Feasibility × Confidence × Time-to-Value.
Recommend one pilot with the smallest integration surface and clearest value proof to the TSF Group.`,
  },
  {
    step: 4,
    label: "Brief",
